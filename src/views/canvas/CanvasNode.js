...
const CanvasNode = ({ data }) => {
    ...
    const addSwitchOutputs = () => {
        let newOutput = {
            "id": `switch-output-case${data.outputAnchors.length}-xMessage`,
            "name": `case-${data.outputAnchors.length}`,
            "label": `Case ${data.outputAnchors.length}`,
            "type": "xMessage"
        }
        ...
        <>
            ...
            >
                <Box>
                    ...
                    <Divider />
                    {data.category === 'SwitchCaseTransformer' && <Button sx={{ borderRadius: 25, width: '90%', mb: 2, ml: 2 }} variant='outlined' onClick={addSwitchOutputs}>
                                Add States
                            </Button>}
