...
export const initNode = (nodeData, newNodeId) => {
    ...
    console.log('nodeData', nodeData.outputs)
...
export const isValidConnection = (connection, reactFlowInstance) => {
    ...
    if (targetTypes.some((t) => sourceTypes.includes(t))) {
        ...
        if (!targetNode) {
            ...
            console.log('targetNodeInputAnchor', targetNodeInputAnchor)
            if (
...
export const generateExportFlowData = (flowData) => {
    ...
    const modifiedEdges = edges.map((edge) => {
        if (edge.sourceHandle) {
            const splitValues = edge.sourceHandle.split('-')
            if (splitValues.length >= 3) {
                return {
                    ...edge,
                    sourceHandle: splitValues[2]
                }
            } else {
                return {
                    ...edge,
